...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const item = await prisma.incomingLetter.findUnique({ where: { id } });
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = incomingLetterSchema.partial().safeParse(body);
  if (!parsed.success) return badRequest("Input tidak valid");
  const updated = await prisma.incomingLetter.update({ where: { id }, data: parsed.data });
  return ok(updated);
}
